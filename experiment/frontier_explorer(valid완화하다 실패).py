...
class FrontierExplorer(Node):
    def __init__(self) -> None:
        ...
        self.blacklist_cells_ts: Dict[Tuple[int, int], float] = {}  # (gx,gy) -> epoch
        self.declare_parameter("blacklist_decay_sec", 60.0)  # n초 지나면 해제
        self.blacklist_decay_sec: float = self.get_parameter(
            "blacklist_decay_sec"
        ).value
    ...
    def plan_and_go(self, grid: OccupancyGrid, use_relaxation: bool = False) -> None:
        ...
        relax_try_levels = [0] + ([1, 2, 3, 4] if use_relaxation else [])
        goal = None
        for rl in relax_try_levels:
            # 단계별 min_frontier 동적 조정
            min_frontier = max(1, self.min_frontier - rl)
            frontiers = self.find_frontiers(grid, min_frontier)
            if not frontiers:
                continue
            goal = self.choose_goal(frontiers, grid, relax_level=rl)
            if goal is not None:
                break

        if goal is None:
            self.get_logger().warn(
                "No valid frontier goal found (blocked or repeated). (tried relax 0..{})".format(
                    (relax_try_levels[-1] if relax_try_levels else 0)
                )
            )
            return
        ...
        if attempts >= self.max_goal_attempts:
            ...
            self._mark_blacklist(gx, gy)  # ← 변경: 타임스탬프 포함
    ...
    def _decay_blacklist(self, relax_level: int):
        now = time.time()
        decay = max(5.0, self.blacklist_decay_sec / max(1, (relax_level + 1)))
        to_del = []
        for cell, ts in self.blacklist_cells_ts.items():
            if now - ts > decay:
                to_del.append(cell)
        for c in to_del:
            self.blacklist_cells.discard(c)
            self.blacklist_cells_ts.pop(c, None)

    def _mark_blacklist(self, gx: int, gy: int):
        self.blacklist_cells.add((gx, gy))
        self.blacklist_cells_ts[(gx, gy)] = time.time()

    def _connected_free(self, grid, rx, ry, gx, gy, max_cells=2000):
        info, data, w, h = grid.info, grid.data, grid.info.width, grid.info.height

        def free(x, y):
            return 0 <= x < w and 0 <= y < h and data[idx(x, y, w)] == 0

        s = world_to_map(rx, ry, info)
        if not (0 <= s[0] < w and 0 <= s[1] < h):
            return False
        if not (0 <= gx < w and 0 <= gy < h):
            return False
        if not (free(s[0], s[1]) and free(gx, gy)):
            return False
        from collections import deque
