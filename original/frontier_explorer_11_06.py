...
class FrontierExplorer(Node):
    def __init__(self) -> None:
        ...
        # 탐사 멈춤 방지 신규 파라미터
        self.declare_parameter("frontier_search_timeout", 5)    # s, 맵 업데이트/프런티어 정체 감지
        self.declare_parameter("goal_timeout_sec", 30.0)           # s, goal 도달 타임아웃
        self.declare_parameter("blacklist_clear_radius", 3)      # m, 블랙리스트 반경(월드)
        self.declare_parameter("retry_distance", 0.5)              # m, 프런티어에서 뒤로 당길 거리
        self.declare_parameter("max_goal_attempts_per_frontier", 2)# 프런티어 재시도 횟수 제한
        ...
        self.base_frame: str = self.get_parameter("robot_base_frame").get_parameter_value().string_value

        self.min_frontier: int = self.get_parameter("min_frontier_size").get_parameter_value().integer_value
        self.clearance: int = self.get_parameter("goal_clearance_cells").get_parameter_value().integer_value
        self.wait_nav2_sec: float = self.get_parameter("wait_nav2_sec").get_parameter_value().double_value

        self.frontier_search_timeout: float = self.get_parameter("frontier_search_timeout").get_parameter_value().double_value
        self.goal_timeout_sec: float = self.get_parameter("goal_timeout_sec").get_parameter_value().double_value
        self.blacklist_clear_radius_m: float = self.get_parameter("blacklist_clear_radius").get_parameter_value().double_value
        self.retry_distance_m: float = self.get_parameter("retry_distance").get_parameter_value().double_value
        self.max_goal_attempts: int = self.get_parameter("max_goal_attempts_per_frontier").get_parameter_value().integer_value
        ...
        # ===== Nav2 action =====
        self.nav_client: ActionClient = ActionClient(self, NavigateToPose, "navigate_to_pose")
        ...
        if not self.nav_client.server_is_ready():
            self.get_logger().warn("Nav2 action server not ready yet; will continue once map arrives.")
    ...
    def _make_param(self, name: str, value) -> ParamMsg:
        ...
        p = ParamMsg(); p.name = name; p.value = v
        return p
    ...
    def watchdog_cb(self) -> None:
        ...
        if self._have_map and self.current_goal_future is None:
            if (now - self._last_map_time).nanoseconds * 1e-9 > self.frontier_search_timeout:
                self.get_logger().warn("[Watchdog] Map stale; retry planning with relaxation.")
    ...
    def get_robot_pose(self) -> Optional[Tuple[float, float, float]]:
        try:
            tf = self.buffer.lookup_transform(
                self.global_frame, self.base_frame, rclpy.time.Time(), Duration(seconds=1.0)
            )
    ...
    # ===== Frontier detection =====
    def find_frontiers(self, grid: OccupancyGrid, min_size: int) -> List[List[Tuple[int, int]]]:
        info = grid.info
        data = grid.data
            ...
            for x in range(w):
                if is_frontier[y][x] and not visited[y][x]:
                    ...
                    while q:
                        ...
                        for dx, dy in nbs4:
                            nx, ny = cx + dx, cy + dy
                            if 0 <= nx < w and 0 <= ny < h and not visited[ny][nx] and is_frontier[ny][nx]:
                                visited[ny][nx] = True
                                q.append((nx, ny))
    ...
    def cell_clear(self, x: int, y: int, grid: OccupancyGrid, clearance_cells: int) -> bool:
        info = grid.info
        w, h = info.width, info.height
        data = grid.data
    ...
    def choose_goal(self, frontiers: List[List[Tuple[int, int]]], grid: OccupancyGrid, relax_level: int = 0):
        ...
        # Relaxation 전략
        clearance_cells = max(0, self.clearance - relax_level)  # 레벨마다 1셀씩 느슨하게
        best = None
        best_dist = 1e9
        ...
        for comp in frontiers:
                ...
                for r in range(1, rmax + 1):
                    for dy in range(-r, r + 1):
                        for dx in range(-r, r + 1):
                            x, y = gx + dx, gy + dy
                            if 0 <= x < w and 0 <= y < h and free(x, y) and self.cell_clear(x, y, grid, clearance_cells):
                                return x, y
                return None
            ...
            if pick is None:
                ...
                fx, fy = min(
                    comp,
                    key=lambda p: (map_to_world(p[0], p[1], info)[0] - rx) ** 2 + (map_to_world(p[0], p[1], info)[1] - ry) ** 2
                )
                ...
                for cmx, cmy in cand_m:
                    ...
                    if 0 <= cgx < w and 0 <= cgy < h and free(cgx, cgy) and self.cell_clear(cgx, cgy, grid, clearance_cells):
                        pick = (cgx, cgy)
                        break
